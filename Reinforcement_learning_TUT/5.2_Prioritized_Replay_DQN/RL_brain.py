...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        if not hasattr(self, 'qn'):     # record max value
            self.qn = []
            self.running_qn = 0
        self.running_qn = self.running_qn*0.95 + 0.05 * np.max(q_next, axis=1).mean()
        self.qn.append(self.running_qn)
...
class SumTree(object):
    ...
    def vision(self):
        ...
        while True:
            i_ = 2*i+1
            print(self.tree[i:i_])
            if i_ >= len(self.tree): break
            i = i_
...
class DoubleDQNPrioritizedReplay:
    def __init__(
            self,
            n_actions,
            n_features,
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('loss'):
            ...
            self.loss = tf.reduce_sum(tf.square(self.ISWeights * self.abs_errors))
    ...
    def learn(self):
        ...
        if not hasattr(self, 'qn'):     # record max value
            self.qn = []
            self.running_qn = 0
        self.running_qn = self.running_qn*0.95 + 0.05 * selected_q_next.mean()
        self.qn.append(self.running_qn)
    ...
    def plot_max_estimation(self):
        plt.plot(np.array(self.qn))
        dqn = '(double DQN)' if self.double_q else '(natural DQN)'
        plt.ylabel('max q next %s' % dqn)
        plt.xlabel('training steps')
        plt.grid()
        plt.show()
