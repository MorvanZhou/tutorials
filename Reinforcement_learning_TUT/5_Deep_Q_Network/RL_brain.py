...
class DeepQNetwork:
    ...
    def choose_action(self, observation):
        # observation can be from a sample or samples
        if type(observation) is list:
            observations = np.array(observation)
            if observations.ndim < 2:
                observation = observations[np.newaxis, :]  # gain the batch dimension
        ...
        if np.random.rand() < self.epsilon:
            ...
            action = np.random.randint(0, self.n_actions, size=batch_size)

        if len(action) == 1:
            # the action in env
            action = action[0]
        else:
            # the batch actions in network updating
            pass
        return action
    ...
    def learn(self):
        ...
        batch_memory = self.memory.sample(32) if self.memory_counter > self.memory_size else self.memory.iloc[:self.memory_counter].sample(32, replace=True)
        ...
        target = q_eval.copy()
        target[np.arange(32, dtype=np.int32), batch_memory.iloc[:, self.n_features].astype(int)] = \
            batch_memory.iloc[:, self.n_features+1] + self.gamma * np.max(q_next, axis=1)

        # train eval network
        _, cost = self.sess.run([self._train_op, self.loss], feed_dict={self.s: batch_memory.iloc[:, :self.n_features], self.q_target: target})
        self.cost_his.append(cost)

        # increasing epsilon
        self.epsilon = self.epsilon + .002 if self.epsilon < 0.9 else self.epsilon
        self.learn_steps += 1

        print("step: ", self.learn_steps, ' epsilon: ', round(self.epsilon, 2))
